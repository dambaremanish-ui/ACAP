...
async function fetchInitialData() {
    ...
    try {
        ...
        allStudents = data.students || []; masterOptions = data.options || []; adminSettings = data.settings || {};
        handleSearch();
    } catch (e) { document.getElementById('results').innerHTML = `<p style="color:#dc2626; font-weight:bold;">Error: ${e.message}</p>`; }
}
...
function renderTable() {
    ...
    if(data.length === 0) {
        document.getElementById('results').innerHTML = '<p>No pending students found within constraints.</p>';
...
function updateDropdowns() {
    ...
    selects.forEach(select => {
        const curr = select.value; select.innerHTML = '<option value="">-- Select --</option>'; 
...
function renderPagination(total) {
    const tPages = Math.ceil(total / ROWS_PER_PAGE); const cId = 'studentPagination';
