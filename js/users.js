...
window.onload = async () => {
    const actionBar = `<input type="text" id="searchBox" placeholder="Search users..." style="flex-grow:1;"><button style="background: #4F46E5;" onclick="openUserModal()">+ Add / Edit User</button><button id="syncBtn">↻ Sync Database</button>`;
    loadMasterLayout('User Management', 'users', actionBar);
...
async function fetchUsers() {
    ...
    try { allUsers = await callAPI('getUsersData'); handleSearch(); } 
...
function renderTable() {
    ...
    filteredUsers.forEach((row, idx) => {
        html += `<tr><td>${idx + 1}</td><td><b>${row.username}</b></td><td>${row.password}</td><td><span class="badge">${row.role}</span></td><td><small>${row.permissions.join(', ')}</small></td><td><button onclick="openUserModal('${row.username}')">Edit</button></td></tr>`;
    });
    document.getElementById('results').innerHTML = html + '</table></div>';
}

function openUserModal(username = null) {
    let uData = { username: '', password: '', role: 'Level 3 - Operator', permissions: [] }, isEdit = false;
    if (username) { let match = allUsers.find(r => r.username === username); if(match) { uData = match; isEdit = true; } }
    let html = `<div style="background:#f8fafc; padding:20px; border-radius:8px; border:1px solid var(--border); margin-bottom:20px; max-width:600px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;"><h4 style="margin:0;">${isEdit ? 'Edit User' : 'Create New User'}</h4><button class="btn-danger" style="margin:0;" onclick="document.getElementById('actionModalUsers').innerHTML=''">Cancel</button></div>
        ...
        <div class="input-group"><label>Role Level Indicator</label><select id="mu_role">
    ...
    document.getElementById('actionModalUsers').innerHTML = html; window.scrollTo({ top: 0, behavior: 'smooth' });
}

async function saveUserModal(e) {
    let u = document.getElementById('mu_user').value.trim(), p = document.getElementById('mu_pass').value.trim(), r = document.getElementById('mu_role').value;
    if(!u || !p) return alert("Username and Password required.");
    let perms = []; document.querySelectorAll('.cb-perm:checked').forEach(cb => perms.push(cb.value));
    e.target.innerText = 'Saving...'; e.target.disabled = true;
    try { await callAPI('saveUser', { username: u, password: p, role: r, permissions: perms }); alert('Saved!'); document.getElementById('actionModalUsers').innerHTML = ''; await fetchUsers();
    } catch(err) { alert("Error: " + err.message); e.target.innerText = 'Save User'; e.target.disabled = false; }
}

async function deleteUserModal(username, e) {
    if(!confirm(`Delete user '${username}'?`)) return;
    e.target.innerText = 'Deleting...'; e.target.disabled = true;
    try { await callAPI('deleteUser', { username: username }); alert('Deleted.'); document.getElementById('actionModalUsers').innerHTML = ''; await fetchUsers();
    } catch (err) { alert('Error: ' + err.message); e.target.innerText = 'Delete User'; e.target.disabled = false; }
}
